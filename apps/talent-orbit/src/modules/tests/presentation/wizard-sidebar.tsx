...
const navItems = [
  ...
  { icon: Users, label: copy.nav.users, active: false },
  { icon: BookOpen, label: copy.nav.knowledgeBank, active: false },
  { icon: ClipboardCheck, label: copy.nav.tests, active: true },
  { icon: FileBarChart2, label: copy.nav.reports, active: false },
];

export const WizardSidebar = () => (
  <aside
    data-e2e="tests:wizard-sidebar"
  ...
  >
      ...
      <nav aria-label={copy.brand.name} className="mt-8 flex flex-col gap-1">
        {navItems.map(({ icon: Icon, label, active }) => (
          <a
            ...
            data-e2e={`tests:wizard-nav-${label.toLowerCase().replace(/\s+/g, '-')}`}
    ...
    <div className="flex flex-col gap-1">
      <a
        href="#"
        data-e2e="tests:wizard-nav-support-center"
      ...
      <a
        href="#"
        data-e2e="tests:wizard-nav-settings"
        className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white"
      >
