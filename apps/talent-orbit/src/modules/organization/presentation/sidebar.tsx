...
const navItems = [
  ...
  { icon: Users, label: copy.nav.assessments, active: false },
  { icon: Building2, label: copy.nav.organizations, active: true },
...
export const Sidebar = () => (
  ...
  >
      ...
      <nav aria-label={copy.brand.name} className="mt-8 flex flex-col gap-1">
        {navItems.map(({ icon: Icon, label, active }) => (
          <a
            ...
            data-e2e={`organization:nav-${label.toLowerCase().replace(/\s+/g, '-')}`}
    ...
    <a
      href="#"
      data-e2e="organization:nav-settings"
