...
const navItems = [
  ...
  { icon: HelpCircle, label: copy.nav.questions, active: true },
  { icon: Users, label: copy.nav.candidates, active: false },
  { icon: UserRoundSearch, label: copy.nav.analytics, active: false },
];

export const ListSidebar = () => (
  <aside
    data-e2e="questions-library:sidebar"
  ...
  >
      ...
      <nav aria-label={copy.brand.name} className="mt-8 flex flex-col gap-1">
        {navItems.map(({ icon: Icon, label, active }) => (
          <a
            ...
            data-e2e={`questions-library:nav-${label.toLowerCase().replace(/\s+/g, '-')}`}
    ...
    <div className="flex flex-col gap-1">
      <a
        href="#"
        data-e2e="questions-library:nav-settings"
      ...
      <a
        href="#"
        data-e2e="questions-library:nav-support-center"
        className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white"
      >
