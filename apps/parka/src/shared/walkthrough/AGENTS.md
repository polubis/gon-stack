...
Generic multi-step walkthrough engine at `src/shared/walkthrough`. Callers pass `WalkthroughStep[]`, mount `<Main />`. No feature copy here. Cloned from [`user-profile-setup`](../../../../romantic-app/src/modules/user-profile-setup) — see that `AGENTS.md` for layering. `__log__/` = clone diffs + task history.
...
7. `__log__` — one file per task, format at `.claude/templates/task-log.md`.
...
- [**log**/](./__log__/) — per-task decisions + progress/time/cost.
