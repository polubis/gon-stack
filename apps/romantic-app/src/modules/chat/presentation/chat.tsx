...
export default function ChatModule() {
  const { threads, messages, threadsSummary, activeThreadHeader, connectionStatus } =
    useChatContext();
        ...
        <aside className="variant-card flex w-full flex-col overflow-hidden lg:max-w-[320px]">
          <header className="variant-nav flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[0.75rem] text-text-tertiary">{threadsSummary.total} total</span>
            </div>
            <button type="button" className="variant-icon-button text-text-secondary" aria-label="Start new thread">
              <Plus size={16} aria-hidden="true" />
            </button>
          </header>

          <div className="flex flex-col gap-2 p-3">
            <label htmlFor="thread-search" className="text-[0.6875rem] uppercase tracking-[0.16em] text-text-tertiary">
              Search
            </label>
              ...
              <li key={thread.id}>
                ...
                >
                  <div className="mb-1 flex items-start justify-between gap-2">
                    <p className="truncate text-[0.875rem] font-semibold">{thread.title}</p>
                    <span className="shrink-0 text-[0.6875rem] text-text-tertiary">{thread.time}</span>
                  </div>
                  <p className="truncate text-[0.75rem] text-text-secondary">{thread.preview}</p>
                  <div className="mt-2 flex items-center gap-2">
                    {thread.unread > 0 ? (
                      <span className="variant-pill text-[0.625rem] tracking-[0.12em]">{thread.unread} New</span>
                    ) : null}
                    {thread.state === 'loading' ? (
                      <span className="inline-flex items-center gap-1 text-[0.6875rem] text-warning" aria-live="polite">
                        <LoaderCircle size={12} className="animate-spin motion-reduce:animate-none" aria-hidden="true" />
                        Syncing…
                      </span>
                    ) : null}
                    {thread.state === 'error' ? (
                      <span className="text-[0.6875rem] text-error">Needs attention</span>
                    ) : null}
                  </div>
                </button>
              </li>
        ...
        <section className="variant-card flex min-h-[70vh] flex-1 flex-col overflow-hidden">
          <header className="variant-nav flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3 md:px-5">
            <div className="min-w-0">
              <h1 className="truncate text-[1rem] font-semibold">{activeThreadHeader.title}</h1>
              <p className="text-[0.75rem] text-text-tertiary">{activeThreadHeader.modelLabel} • {activeThreadHeader.contextStatus}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button type="button" className="variant-button-ghost inline-flex items-center gap-1 px-3 py-2 text-[0.75rem] font-medium">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-3 py-4 md:px-5">
            {messages.map((message) => {
              if (message.role === 'system') {
                return (
                  <div key={message.id} className="mx-auto max-w-xl rounded-full border border-info/35 bg-info/10 px-4 py-2 text-center">
                    <p className="text-[0.75rem] text-text-secondary">{message.body}</p>
                  </div>
                );
              }
            ...
            <div className="mr-auto max-w-[80%]">
              <div className="rounded-2xl border border-secondary-300/40 bg-surface-50/80 px-4 py-3">
                <p className="mb-2 text-[0.75rem] uppercase tracking-[0.14em] text-text-tertiary">Assistant is thinking</p>
          ...
          <footer className="border-t border-white/10 bg-surface-100/50 p-3 md:p-4">
            <div className="flex items-end gap-2">
              ...
              />
              <button type="button" className="variant-button-primary inline-flex h-[42px] items-center gap-1 px-4 text-[0.875rem] font-semibold">
