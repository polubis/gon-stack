    ...
    <div className="px-4 flex justify-center">
      <div className="w-full max-w-lg flex flex-col gap-4">
        ...
        <div className="variant-card p-4 text-center">
          <p className="text-success text-sm font-semibold">Perfect match! +1 point</p>
        </div>
        ...
        <div className="variant-card p-4">
          ...
          <button onClick={() => setExpanded((v) => !v)} className="mt-3 text-xs font-semibold text-info">
      ...
      <div className="w-full max-w-4xl grid md:grid-cols-2 gap-4">
        <div className="variant-card p-5 flex flex-col gap-2">
          ...
          <p className="text-base text-text-secondary">
            Body / p1: use for question prompts, card copy, and conversational guidance text.
          </p>
          <p className="text-sm text-text-secondary">
            Body-sm / p2: use for secondary descriptions, helper hints, and supporting UI text.
          </p>
          <p className="text-xs text-text-tertiary">
            Caption: metadata, timestamps, subtle status notes, and non-critical annotations.
          </p>
        ...
        <div className="variant-card p-5 flex flex-col gap-3 md:col-span-2">
          <p className="text-xs uppercase tracking-[0.16em] text-text-tertiary">Primary palette</p>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
            {primarySwatches.map((swatch) => (
              <div key={swatch.label} className={`rounded-md ${swatch.className} h-10 flex items-end p-1`}>
                <span className="text-[10px] text-text-primary/80">{swatch.label}</span>
              </div>
            ))}
          </div>
          <p className="text-xs uppercase tracking-[0.16em] text-text-tertiary">Secondary palette</p>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
    ...
    <div className="variant-shell min-h-screen text-text-primary" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            {navItems.map((n) => (
              <a
                ...
                className={[
                  ...
                  n === variantNumber ? 'variant-nav-active' : 'variant-nav-item',
                ].join(' ')}
              >
                /{n}
              </a>
      ...
      <header className="variant-hero border-b border-surface-200">
        <div className="max-w-5xl mx-auto px-4 py-10 text-center">
          <p className="text-xs uppercase tracking-[0.22em] text-text-tertiary mb-2">Romantic app ui system</p>
